...
let normalize  (steps : list norm_step) (tm:term) : Tac unit =
  dup ();
  exact tm;
  norm steps;
  trefl ()

let def_of (t:term) : Tac term =
  let e = cur_env () in
...
let does_not_normalize (#t:Type) (x:t) : t =
  synth_by_tactic #t (fun () -> normalize [primops; delta] (quote x))
