  ...
  fun x  ->
    let basic =
      ...
      | FStar_Syntax_Syntax.Sig_inductive_typ
          (lid,univs1,tps,k,uu____3943,uu____3944) ->
          let quals_str = quals_to_string' x.FStar_Syntax_Syntax.sigquals  in
          let binders_str = binders_to_string " " tps  in
          let term_str = term_to_string k  in
          let uu____3960 = FStar_Options.print_universes ()  in
          if uu____3960
          then
            let uu____3964 = univ_names_to_string univs1  in
            FStar_Util.format5 "%stype %s<%s> %s : %s" quals_str
              lid.FStar_Ident.str uu____3964 binders_str term_str
          else
            FStar_Util.format4 "%stype %s %s : %s" quals_str
              lid.FStar_Ident.str binders_str term_str
      | FStar_Syntax_Syntax.Sig_datacon
          (lid,univs1,t,uu____3973,uu____3974,uu____3975) ->
          let uu____3982 = FStar_Options.print_universes ()  in
          if uu____3982
          then
            let uu____3986 = univ_names_to_string univs1  in
            let uu____3988 = term_to_string t  in
            FStar_Util.format3 "datacon<%s> %s : %s" uu____3986
              lid.FStar_Ident.str uu____3988
          else
            (let uu____3993 = term_to_string t  in
             FStar_Util.format2 "datacon %s : %s" lid.FStar_Ident.str
               uu____3993)
      | FStar_Syntax_Syntax.Sig_declare_typ (lid,univs1,t) ->
          let uu____3999 = quals_to_string' x.FStar_Syntax_Syntax.sigquals
             in
          let uu____4001 =
            let uu____4003 = FStar_Options.print_universes ()  in
            if uu____4003
            then
              let uu____4007 = univ_names_to_string univs1  in
              FStar_Util.format1 "<%s>" uu____4007
            else ""  in
          let uu____4013 = term_to_string t  in
          FStar_Util.format4 "%sval %s %s : %s" uu____3999
            lid.FStar_Ident.str uu____4001 uu____4013
      | FStar_Syntax_Syntax.Sig_assume (lid,us,f) ->
          let uu____4019 = FStar_Options.print_universes ()  in
          if uu____4019
          then
            let uu____4023 = univ_names_to_string us  in
            let uu____4025 = term_to_string f  in
            FStar_Util.format3 "val %s<%s> : %s" lid.FStar_Ident.str
              uu____4023 uu____4025
          else
            (let uu____4030 = term_to_string f  in
             FStar_Util.format2 "val %s : %s" lid.FStar_Ident.str uu____4030)
      | FStar_Syntax_Syntax.Sig_let (lbs,uu____4034) ->
          lbs_to_string x.FStar_Syntax_Syntax.sigquals lbs
      | FStar_Syntax_Syntax.Sig_main e ->
          let uu____4040 = term_to_string e  in
          FStar_Util.format1 "let _ = %s" uu____4040
      | FStar_Syntax_Syntax.Sig_bundle (ses,uu____4044) ->
          let uu____4053 =
            let uu____4055 = FStar_List.map sigelt_to_string ses  in
            FStar_All.pipe_right uu____4055 (FStar_String.concat "\n")  in
          Prims.op_Hat "(* Sig_bundle *)" uu____4053
      ...
      | FStar_Syntax_Syntax.Sig_sub_effect se ->
          let lift_wp =
            ...
            | (FStar_Pervasives_Native.None ,FStar_Pervasives_Native.None )
                -> failwith "impossible"
            | (FStar_Pervasives_Native.Some lift_wp,uu____4100) -> lift_wp
            | (uu____4107,FStar_Pervasives_Native.Some lift) -> lift  in
          let uu____4115 =
          ...
          (match uu____4115 with
           | (us,t) ->
               let uu____4127 = lid_to_string se.FStar_Syntax_Syntax.source
                  in
               let uu____4129 = lid_to_string se.FStar_Syntax_Syntax.target
                  in
               let uu____4131 = univ_names_to_string us  in
               let uu____4133 = term_to_string t  in
               FStar_Util.format4 "sub_effect %s ~> %s : <%s> %s" uu____4127
                 uu____4129 uu____4131 uu____4133)
      | FStar_Syntax_Syntax.Sig_effect_abbrev (l,univs1,tps,c,flags) ->
          let uu____4145 = FStar_Options.print_universes ()  in
          if uu____4145
          then
            let uu____4149 =
              let uu____4154 =
                FStar_Syntax_Syntax.mk
                 ...
                 in
              FStar_Syntax_Subst.open_univ_vars univs1 uu____4154  in
            (match uu____4149 with
             | (univs2,t) ->
                 let uu____4168 =
                   ...
                   | uu____4203 -> failwith "impossible"  in
                 (match uu____4168 with
                  | (tps1,c1) ->
                      let uu____4212 = sli l  in
                      let uu____4214 = univ_names_to_string univs2  in
                      let uu____4216 = binders_to_string " " tps1  in
                      let uu____4219 = comp_to_string c1  in
                      FStar_Util.format4 "effect %s<%s> %s = %s" uu____4212
                        uu____4214 uu____4216 uu____4219))
          else
            (let uu____4224 = sli l  in
             let uu____4226 = binders_to_string " " tps  in
             let uu____4229 = comp_to_string c  in
             FStar_Util.format3 "effect %s %s = %s" uu____4224 uu____4226
               uu____4229)
      | FStar_Syntax_Syntax.Sig_splice (lids,t) ->
          let uu____4238 =
            let uu____4240 = FStar_List.map FStar_Ident.string_of_lid lids
               in
            FStar_All.pipe_left (FStar_String.concat "; ") uu____4240  in
          let uu____4250 = term_to_string t  in
          FStar_Util.format2 "splice[%s] (%s)" uu____4238 uu____4250
       in
    match x.FStar_Syntax_Syntax.sigattrs with
    | [] -> basic
    | uu____4254 ->
        let uu____4257 = attrs_to_string x.FStar_Syntax_Syntax.sigattrs  in
        Prims.op_Hat uu____4257 (Prims.op_Hat "\n" basic)
  
let (format_error : FStar_Range.range -> Prims.string -> Prims.string) =
  fun r  ->
    fun msg  ->
      let uu____4274 = FStar_Range.string_of_range r  in
...
let rec (sigelt_to_string_short : FStar_Syntax_Syntax.sigelt -> Prims.string)
  ...
  fun x  ->
    ...
    | FStar_Syntax_Syntax.Sig_let
        ((uu____4285,{ FStar_Syntax_Syntax.lbname = lb;
                       FStar_Syntax_Syntax.lbunivs = uu____4287;
                       FStar_Syntax_Syntax.lbtyp = t;
                       FStar_Syntax_Syntax.lbeff = uu____4289;
                       FStar_Syntax_Syntax.lbdef = uu____4290;
                       FStar_Syntax_Syntax.lbattrs = uu____4291;
                       FStar_Syntax_Syntax.lbpos = uu____4292;_}::[]),uu____4293)
        ->
        let uu____4316 = lbname_to_string lb  in
        let uu____4318 = term_to_string t  in
        FStar_Util.format2 "let %s : %s" uu____4316 uu____4318
    | uu____4321 ->
        let uu____4322 =
          FStar_All.pipe_right (FStar_Syntax_Util.lids_of_sigelt x)
            (FStar_List.map (fun l  -> l.FStar_Ident.str))
           in
        FStar_All.pipe_right uu____4322 (FStar_String.concat ", ")
  
let rec (modul_to_string : FStar_Syntax_Syntax.modul -> Prims.string) =
  fun m  ->
    let uu____4346 = sli m.FStar_Syntax_Syntax.name  in
    let uu____4348 =
      let uu____4350 =
    ...
    let uu____4360 =
      ...
      FStar_All.pipe_right uu____4362 (FStar_String.concat "\n")  in
    FStar_Util.format3
      "module %s\nDeclarations: [\n%s\n]\nExports: [\n%s\n]\n" uu____4346
      uu____4348 uu____4360
     ...
     | FStar_Pervasives_Native.Some (FStar_Util.Inl lc) ->
         (FStar_Util.string_builder_append strb "Some Inr ";
          (let uu____4406 =
           ...
           FStar_Util.string_builder_append strb uu____4406))
     | FStar_Pervasives_Native.Some (FStar_Util.Inr lid) ->
         (FStar_Util.string_builder_append strb "Some Inr ";
          (let uu____4415 = FStar_Ident.text_of_lid lid  in
    ...
    fun elts  ->
      ...
      | x::xs ->
          ...
          (FStar_Util.string_builder_append strb "[";
           (let uu____4456 = f x  in
            FStar_Util.string_builder_append strb uu____4456);
           FStar_List.iter
             (fun x1  ->
                FStar_Util.string_builder_append strb "; ";
                (let uu____4465 = f x1  in
                 FStar_Util.string_builder_append strb uu____4465)) xs;
           FStar_Util.string_builder_append strb "]";
           FStar_Util.string_of_string_builder strb)
...
let (bvs_to_string :
  ...
  fun sep  ->
    fun bvs  ->
      let uu____4543 = FStar_List.map FStar_Syntax_Syntax.mk_binder bvs  in
      binders_to_string sep uu____4543
  
let rec (emb_typ_to_string : FStar_Syntax_Syntax.emb_typ -> Prims.string) =
  fun uu___13_4556  ->
    match uu___13_4556 with
    ...
    | FStar_Syntax_Syntax.ET_app (h,args) ->
        let uu____4572 =
          let uu____4574 =
            let uu____4576 =
              let uu____4578 =
                let uu____4580 = FStar_List.map emb_typ_to_string args  in
                FStar_All.pipe_right uu____4580 (FStar_String.concat " ")  in
              Prims.op_Hat uu____4578 ")"  in
            Prims.op_Hat " " uu____4576  in
          Prims.op_Hat h uu____4574  in
        Prims.op_Hat "(" uu____4572
    | FStar_Syntax_Syntax.ET_fun (a,b) ->
        let uu____4595 =
          let uu____4597 = emb_typ_to_string a  in
          let uu____4599 =
            let uu____4601 = emb_typ_to_string b  in
            Prims.op_Hat ") -> " uu____4601  in
          Prims.op_Hat uu____4597 uu____4599  in
        Prims.op_Hat "(" uu____4595
