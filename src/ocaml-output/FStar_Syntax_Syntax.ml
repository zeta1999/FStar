...
let uu___is_TypeAbbrev : qualifier -> Prims.bool =
  fun projectee  ->
    match projectee with | TypeAbbrev  -> true | uu____2554 -> false
...
let uu___is_Sig_inductive_typ : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_inductive_typ _0 -> true | uu____2957 -> false
...
let uu___is_Sig_bundle : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_bundle _0 -> true | uu____2997 -> false
...
let uu___is_Sig_datacon : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_datacon _0 -> true | uu____3028 -> false
...
let uu___is_Sig_declare_typ : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_declare_typ _0 -> true | uu____3064 -> false
...
let uu___is_Sig_let : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_let _0 -> true | uu____3090 -> false
...
let uu___is_Sig_main : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_main _0 -> true | uu____3117 -> false
...
let uu___is_Sig_assume : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_assume _0 -> true | uu____3131 -> false
...
let uu___is_Sig_new_effect : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_new_effect _0 -> true | uu____3149 -> false
...
let uu___is_Sig_sub_effect : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_sub_effect _0 -> true | uu____3173 -> false
...
let uu___is_Sig_effect_abbrev : sigelt' -> Prims.bool =
  fun projectee  ->
    match projectee with | Sig_effect_abbrev _0 -> true | uu____3191 -> false
...
let contains_reflectable : qualifier Prims.list -> Prims.bool =
  fun l  ->
    FStar_Util.for_some
      (fun uu___87_3302  ->
         match uu___87_3302 with
         | Reflectable uu____3303 -> true
         | uu____3304 -> false) l
...
let set_range_of_bv : bv -> FStar_Range.range -> bv =
  fun x  ->
    fun r  ->
      let uu___94_3380 = x  in
      {
        ppname = (FStar_Ident.mk_ident (((x.ppname).FStar_Ident.idText), r));
        index = (uu___94_3380.index);
        sort = (uu___94_3380.sort)
      }
  
let syn p k f = f k p 
let mk_fvs uu____3421 = FStar_Util.mk_ref FStar_Pervasives_Native.None 
let mk_uvs uu____3433 = FStar_Util.mk_ref FStar_Pervasives_Native.None 
    ...
    FStar_Util.new_set
      (fun uu____3459  ->
         fun uu____3460  ->
           ...
           | ((x,uu____3494),(y,uu____3496)) ->
               let uu____3537 = FStar_Unionfind.uvar_id x  in
               let uu____3541 = FStar_Unionfind.uvar_id y  in
               uu____3537 - uu____3541)
      (fun uu____3545  ->
         match uu____3545 with | (x,uu____3555) -> FStar_Unionfind.uvar_id x)
...
let new_universe_uvar_set :
  ...
  fun uu____3574  ->
    FStar_Util.new_set
      (fun x  ->
         fun y  ->
           let uu____3584 = FStar_Unionfind.uvar_id x  in
           let uu____3586 = FStar_Unionfind.uvar_id y  in
           uu____3584 - uu____3586) (fun x  -> FStar_Unionfind.uvar_id x)
...
let mk t topt r =
  let uu____3659 = FStar_Util.mk_ref topt  in
  let uu____3663 = FStar_Util.mk_ref FStar_Pervasives_Native.None  in
  { n = t; tk = uu____3659; pos = r; vars = uu____3663 } 
let bv_to_tm : bv -> (term',term') syntax =
  fun bv  ->
    let uu____3674 = range_of_bv bv  in
    mk (Tm_bvar bv) (FStar_Pervasives_Native.Some ((bv.sort).n)) uu____3674
  
let bv_to_name : bv -> (term',term') syntax =
  fun bv  ->
    let uu____3682 = range_of_bv bv  in
    mk (Tm_name bv) (FStar_Pervasives_Native.Some ((bv.sort).n)) uu____3682
    ...
    fun args  ->
      fun k  ->
        fun p  ->
          match args with
          | [] -> t1
          | uu____3703 -> mk (Tm_app (t1, args)) k p
  
let mk_Tm_uinst : term -> universes -> term =
  fun t  ->
    fun uu___88_3715  ->
      match uu___88_3715 with
      | [] -> t
      | us ->
          (match t.n with
           | Tm_fvar uu____3717 ->
               mk (Tm_uinst (t, us)) FStar_Pervasives_Native.None t.pos
           | uu____3722 -> failwith "Unexpected universe instantiation")
    ...
    fun args'  ->
      fun kopt  ->
        fun r  ->
          ...
          | uu____3760 -> mk_Tm_app t args' kopt r
  ...
  fun lr  ->
    fun pos  ->
      let uu____3797 =
        let uu____3800 =
          let uu____3801 =
            let uu____3822 = FStar_Util.mk_ref FStar_Pervasives_Native.None
               in
            (lr, uu____3822)  in
          Tm_delayed uu____3801  in
        mk uu____3800  in
      uu____3797 FStar_Pervasives_Native.None pos
...
let null_binder :
  ...
  =
  fun t  ->
    let uu____3964 = null_bv t  in (uu____3964, FStar_Pervasives_Native.None)
...
let is_implicit : aqual -> Prims.bool =
  fun uu___90_4036  ->
    match uu___90_4036 with
    | FStar_Pervasives_Native.Some (Implicit uu____4037) -> true
    | uu____4038 -> false
...
let pat_bvs : pat -> bv Prims.list =
  fun p  ->
    let rec aux b p1 =
      match p1.v with
      | Pat_dot_term uu____4063 -> b
      | Pat_constant uu____4068 -> b
      | Pat_wild x -> x :: b
      | Pat_var x -> x :: b
      | Pat_cons (uu____4071,pats) ->
          FStar_List.fold_left
            (fun b1  ->
               fun uu____4089  ->
                 match uu____4089 with | (p2,uu____4097) -> aux b1 p2) b pats
       in
...
let freshen_bv : bv -> bv =
  fun bv  ->
    ...
    then
      let uu____4178 =
        let uu____4180 = range_of_bv bv  in
        FStar_Pervasives_Native.Some uu____4180  in
      new_bv uu____4178 bv.sort
    else
      (let uu___95_4182 = bv  in
       let uu____4183 = next_id ()  in
       {
         ppname = (uu___95_4182.ppname);
         index = uu____4183;
         sort = (uu___95_4182.sort)
       })
...
let set_bv_range : bv -> FStar_Range.range -> bv =
  fun bv  ->
    fun r  ->
      let uu___96_4275 = bv  in
      {
        ppname = (FStar_Ident.mk_ident (((bv.ppname).FStar_Ident.idText), r));
        index = (uu___96_4275.index);
        sort = (uu___96_4275.sort)
      }
...
let set_range_of_fv : fv -> FStar_Range.range -> fv =
  fun fv  ->
    fun r  ->
      let uu___97_4336 = fv  in
      let uu____4337 =
        let uu___98_4341 = fv.fv_name  in
        let uu____4346 =
          let uu____4347 = lid_of_fv fv  in
          FStar_Ident.set_lid_range uu____4347 r  in
        { v = uu____4346; ty = (uu___98_4341.ty); p = (uu___98_4341.p) }  in
      {
        fv_name = uu____4337;
        fv_delta = (uu___97_4336.fv_delta);
        fv_qual = (uu___97_4336.fv_qual)
      }
