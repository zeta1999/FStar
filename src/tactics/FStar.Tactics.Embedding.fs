...
let e_exn : embedding<exn> =
    let embed_exn (e:exn) (rng:Range.range) _ _ : term =
        ...
        | EExn t ->
            { t with pos = rng }
        | _ ->
            failwith (BU.format1 "cannot embed exn : %s" (BU.message_of_exn e))
    in
    let unembed_exn (t:term) w _ : option<exn> =
        ...
        | _ ->
            if w then
                Err.log_issue t.pos (Err.Warning_NotEmbedded, (BU.format1 "Not a (known) embedded exception: %s" (Print.term_to_string t)));
            None
    in
    mk_emb_full
        embed_exn
