...
open FStar.Monotonic.ManualHeap
friend FStar.Monotonic.ManualHeap
...
let heap_rel (h1:heap) (h2:heap) =
  ...
  prefix (pos_of h1) (pos_of h2) /\
  frozen_on (pos_of h1) h1 h2
...
let test (h1 h2 h3 : heap) (_ : squash (heap_rel h1 h2)) (_ : squash (heap_rel h2 h3)) =
  //assert (heap_rel h1 h1);
  assert (heap_rel h1 h3);
  ()

let test2 (h1 h2 h3 : heap) (_ : (heap_rel h1 h2)) (_ : heap_rel h2 h3) =
...
private assume val gst_witness: p:heap_predicate -> GST unit (fun post h0 -> stable p /\ p h0 /\ (witnessed p ==> post () h0))
private assume val gst_recall:  p:heap_predicate -> GST unit (fun post h0 -> stable p /\ witnessed p /\ (p h0 ==> post () h0))
...
val witness: (#a:Type) -> (#rel:preorder a) -> (r:mref a rel) -> q:(a -> prop) -> ST unit
...
val recall : (#a:Type) -> (#rel:preorder a) -> (r:mref a rel) -> q:(a -> prop) -> ST unit
...
abstract let read (#a:Type) (#rel:preorder a) (r:mref a rel) :STATE a (fun p h -> h `contains` r /\ p (sel h r) h)
  = let h0 = gst_get () in
...
abstract let write (#a:Type) (#rel:preorder a) (r:mref a rel) (v:a)
  : ST unit
    (fun h -> rel (sel h r) v /\ h `contains` r /\ prefix (pos_of h) (fst (addr_of r)))
