...
val freed_in: #a:Type0 -> #rel:preorder a -> mref a rel -> heap -> Type0
...
val lemma_ref_freed_iff_addr_freed (#a:Type0) (#rel:preorder a) (h:heap) (r:mref a rel)
  :Lemma (requires True)
         (ensures  (r `freed_in` h <==> addr_of r `addr_freed_in` h))
         [SMTPatOr [[SMTPat (r `freed_in` h)]; [SMTPat (addr_of r `addr_freed_in` h)]]]
...
val lemma_free_unused
  ...
  :Lemma (requires True)
         (ensures  (let h1 = free h0 r1 in
                    ((addr_of r1 = addr_of r2 ==> r2 `freed_in` h1)      /\
                     (r2 `unused_in` h0      <==> r2 `unused_in` h1)      /\
                     (r2 `freed_in` h0        ==> r2 `freed_in` h1))))
         [SMTPat (r2 `unused_in` (free h0 r1))]

val lemma_free_not_contained
  (#a:Type0) (#rel:preorder a) (h:heap) (r:mref a rel)
  :Lemma (requires (r `freed_in` h))
...
val lemma_unused_upd_freed_in
  (#a:Type0) (#b:Type0) (#rel1:preorder a) (#rel2:preorder b) (h:heap) (r1:mref a rel1) (x:a) (r2:mref b rel2)
  :Lemma (requires (r1 `unused_in` h))
         (ensures  (let h1 = upd h r1 x in
                    (r2 `freed_in` h  ==> r2 `freed_in` h1) /\
                    (r2 `freed_in` h1 ==> (r2 `freed_in` h \/ addr_of r2 = addr_of r1))))
         [SMTPat (r2 `freed_in` (upd h r1 x))]
...
val lemma_upd_freed
  (#a:Type0) (#b:Type0) (#rel1:preorder a) (#rel2:preorder b) (h:heap) (r1:mref a rel1) (x:a) (r2:mref b rel2)
  :Lemma (requires True)
         (ensures  ((addr_of r1 <> addr_of r2 /\ r2 `freed_in` h) <==> r2 `freed_in` (upd h r1 x)))
         [SMTPat (r2 `freed_in` (upd h r1 x))]
